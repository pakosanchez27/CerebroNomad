                        ...
                        @foreach ($users as $user)
                            <div
                                ...
                                <div class="acciones d-flex flex-column gap-2    ">
                                    <a href="{{route('roles.edit', $user->id)}}" class="btn btn-warning"><svg xmlns="http://www.w3.org/2000/svg"
                                            width="20" height="20" fill="white" class="bi bi-pencil-square"
                                            viewBox="0 0 16 16">
                                            <path
                                        ...
                                        <form action="{{ route('roles.destroy', $user->id) }}" method="POST">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-danger" onclick="return confirm('¿Estás seguro de eliminar al colaborador {{ $user->name }}?');">
                                        ...
                                        <form id="resetPasswordForm{{ $user->id }}" action="{{ route('roles.resetPassword', $user->id) }}" method="POST">
                                            @csrf
                                            @method('PUT')
                                            <button type="submit" class="btn btn-info" onclick="return confirm('¿Estás seguro de que deseas restablecer la contraseña del colaborador: {{ $user->name }}?')">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="white" class="bi bi-key-fill" viewBox="0 0 16 16">
